...
- `docs/guide/video/` 目录为存档用途，保留历史文档与素材
- 该目录不参与路由注册与侧边栏配置，亦不会被自动扫描
- 配置依据：`rspress.config.ts` 中 `route.exclude: ['**/guide/video/**']`
- 如需启用，请移除上述排除并在 `themeConfig.sidebar` 中手动配置路由
